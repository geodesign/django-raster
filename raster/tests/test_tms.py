...
class RasterLayerParserWithoutCeleryTests(TestCase):
    ...
    def test_tms_nonexisting_layer(self):
        url = reverse('tms', kwargs={'z': 0, 'y': 0, 'x': 0, 'layers': 'raster_nonexistent.tif', 'format': '.png'})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)

    def test_tms_nonexisting_tile(self):
        url = reverse('tms', kwargs={'z': 100, 'y': 0, 'x': 0, 'layers': 'raster.tif', 'format': '.png'})
        response = self.client.get(url)
    ...
    def test_tms_existing_tile(self):
        # Get tile
        tile = self.rasterlayer.rastertile_set.filter(is_base=False).first()

        # Setup url
        url = reverse('tms', kwargs={'z': tile.tilez, 'y': tile.tiley, 'x': tile.tilex, 'layers': 'raster.tif', 'format': '.png'})
        ...
        leg = Legend.objects.create(title='Legend')
