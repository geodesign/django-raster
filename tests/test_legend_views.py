...
class RasterLegendViewTests(RasterTestCase):
    def test_tms_legend_url_from_layer_id(self):
        ...
        self.assertEqual(json.loads(response.content.strip().decode()), [{"color": "#123456", "expression": "4", "name": "Earth"}])
    ...
    def test_tms_legend_url_from_legend_id(self):
        ...
        response = self.client.get(url)
        self.assertEqual(json.loads(response.content.strip().decode()), [{"color": "#123456", "expression": "10", "name": "Earth"}, {"color": "#654321", "expression": "2", "name": "Wind"}])
        self.assertEqual(response.status_code, 200)
